...
The binary embeds the YAML manifests under `qualitygate/rules/`. `core` contains line endings, commit subjects and diff size; `shared` contains test naming, parameterization suggestions, comment language and source declarations. These packages are always available. Selecting `lang-java` or `lang-python` in `rulesets` also exposes `junit-naming` or `pytest-naming`, with language routing supplied by the package.
...
`dependency_resolution`, `external_provenance`, `ai_only` scopes and commit-to-entity trailer association are not yet implemented. A rule requiring those capabilities, including `then.require_dependency`, remains incomplete. The engine does not downgrade the binding or infer a dependency from import text. Their full implementation remains in the [requirement ledger](implementation.md).
