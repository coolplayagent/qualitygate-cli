...
The matching executable reads `references/rules/{core,shared,lang-java,lang-python,lang-typescript,lang-go,lang-c,lang-cpp}`
...
[Issue 13](https://github.com/coolplayagent/qualitygate-cli/issues/13) adds three opt-in Rust `source-pattern` review signals with fixed `languages: [rust]`. `rust-extern-without-abi` warns on changed `extern {` or `unsafe extern {` lines without an ABI string. `rust-untrusted-dynamic-library-loading` flags nonliteral first arguments at simple `dlopen` or `Library::new/load` calls; a nonliteral path does not establish attacker control. `rust-unsafe-block-in-macro-definition` warns when a `macro_rules!` definition and an `unsafe {` block share a changed line; it misses multi-line definitions. The three patterns accept `prohibited_patterns.rust` and `paths` overrides. Comments, strings, aliases and more complex Rust syntax may produce false positives or false negatives. Use the [Clippy ratchet](diagnostic-ratchets.md#clippy-from-captured-stdout) for semantic linting; malformed selected text and analysis budget exhaustion remain incomplete.

[Issue 14](https://github.com/coolplayagent/qualitygate-cli/issues/14) adds twelve opt-in `lang-typescript` source patterns covering direct and implied evaluation, debugger and dialog calls, loose equality, native prototype mutation, direct prototype-method calls, `Math.random`, `postMessage`, commented code and contact markers in comments. Select `rulesets: [lang-typescript]` and individual rules; each fixes `languages: [typescript]` and scans changed `.ts`, `.tsx`, `.js`, `.jsx`, `.mjs` and `.cjs` lines. `paths` and `prohibited_patterns.typescript` remain configurable. These are lexical review signals: a `postMessage` call may use an exact origin, `Math.random` may serve a nonsecurity purpose, and a comment or string can match other patterns. The comment rules recognize only same-line forms. The [ESLint ratchet](diagnostic-ratchets.md#eslint-from-captured-stdout) provides semantic lint findings on the project's declared target scope. Invalid selected text or exhausted scanner budgets remain incomplete.

[Issue 15](https://github.com/coolplayagent/qualitygate-cli/issues/15) adds twelve opt-in `lang-go` source patterns for same-line SQL formatting or concatenation, `math/rand`, literal TLS bypass, credential literals, insecure SSH host keys, world-writable file modes, panic calls, cgo C allocations, relative and dot imports, sensitive log names, and floating loop counters. Select `rulesets: [lang-go]` and individual rules; each fixes `languages: [go]` and scans changed `.go` lines. `paths` and `prohibited_patterns.go` remain configurable. The patterns are lexical review signals. They cannot prove taint flow, a secret's validity, whether a panic occurs in an exported function, whether a C allocation is freed, or whether a log value is sensitive. The [golangci-lint ratchet](diagnostic-ratchets.md#golangci-lint-v2-from-captured-stdout) provides semantic analyzer findings on the declared Go target scope.
...
[Issue 19](https://github.com/coolplayagent/qualitygate-cli/issues/19) adds nine opt-in `lang-cpp` changed-line review signals for `realloc`, `alloca`, unbounded C string calls, pointer throws, value catches, direct lock calls, `return std::move`, `rand`, and legacy empty `throw()` specifications. Each fixes `languages: [cpp]` and accepts `paths` plus `prohibited_patterns.cpp` overrides. `.h` remains classified as C text; use `.hh`, `.hpp` or `.hxx` for C++ headers or configure a repository-specific rule. The [triage](cpp-rule-triage.md) records the semantic proposals left to analyzers, while the [SARIF guide](c-family-ratchet.md) shows a supported C++ analyzer reference. These patterns are review prompts; comments, strings and naming can yield matches without a semantic defect.
...
[Issue 20](https://github.com/coolplayagent/qualitygate-cli/issues/20) adds twelve optional `lang-java` same-line source reviews for thread stop/yield, explicit GC, finalizers, implicit charset/locale conversion, random and weak crypto APIs, empty catch and abrupt finally exits, SQL concatenation, and direct Runtime.exec calls. All fix `languages: [java]` and scan selected changed `.java` lines. Configure `paths` and `prohibited_patterns.java`; [triage and false-positive limits](java-rule-triage.md) describe the narrower signals and semantic checks assigned to project analyzers.
