...
| §3.1 source declarations, matching dependencies, naming, parameterization, comments, line endings, commits, imports and module boundaries | Syntax rules, Maven effective-model/tree pairing, marker retention, compiled undeclared dependency usage and module dependency directions; Skill-owned built-in YAML is loaded at runtime rather than compiled, while `custom_rules` is loaded from the selected project-policy snapshot. Built-ins carry validated references to the external standards archive, whose schema and `standards_registry_requires_declared_breadth_and_input_coverage` test require every declared organization/language/SDLC/concern lane to reach a direct lifecycle input, validate taxonomy/status semantics, and validate rule mappings. Schema-v5 source-local controls prevent a rule from inventing a control under an archived source (`builtins_reject_controls_not_declared_by_the_archived_source`), while `embedded_archive_keeps_the_required_research_lanes` prevents the requested eight-company/full-lifecycle archive from being narrowed and `builtins_reject_language_scopes_that_mismatch_lifecycle_inputs` prevents a language-specific packaged rule from claiming a different or universal lifecycle lane. | Partial: additional ecosystems and access boundaries remain |
