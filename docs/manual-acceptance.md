...
Budgets are 256 KiB per trust store, 64 trusted keys, 1 MiB per envelope, 128 selected record files and 8 MiB total external input. Identifier lists are limited to 1,024 entries. Loading and signature verification have 30-second budgets; each manual check's `timeout_seconds` can further reduce its verification deadline. Non-regular files, symlinks, path traversal and size overruns are rejected.

This implements manual acceptance records. Agent-run provenance, AI-only declaration scope and commit-to-entity attribution remain separate capabilities; a manual approval does not establish them.
