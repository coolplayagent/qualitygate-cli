...
| Issue 35 command-specific prerequisites, exact init argv, snapshot policy, per-check tool dependencies, bounded structured errors | `tests/init/prerequisites.rs`, `domain::prerequisites::tests`; runtime integrity remains covered by `tests/execution.rs` |
