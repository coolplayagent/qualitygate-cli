...
| Issue 35 missing-policy guidance and safe Windows path display | `tests/init.rs::first_run_errors_honor_formats_and_show_an_action`, `missing_custom_configuration_is_carried_into_the_command`, `snapshot_guidance_depends_on_local_policy_and_selection`, `unreadable_or_malformed_configuration_does_not_suggest_init`; `paths::tests::displays_ordinary_windows_paths_without_verbatim_prefix` (Windows) |
