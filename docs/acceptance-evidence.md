...
| 10. Ecosystem extension | `adapters/syntax/syntax_tests.rs::parses_framework_tests_and_annotations_in_java_python_rust_go_and_typescript`; `tests/custom_rules.rs::private_rule_runs_on_java_and_python_with_stable_diagnostics_and_snapshot_isolation`; `tests/python.rs::real_python_extra_dependency_marker_retention_and_test_repair`; Java/Maven evidence in `tests/maven.rs`; `config_tests.rs::embedded_archive_keeps_the_required_research_lanes`, `builtins_reject_language_scopes_that_mismatch_lifecycle_inputs`; `config/discovery/tests.rs::language_scoped_builtins_are_not_advertised_outside_lifecycle_lanes` | Verified for documented syntax and Java/Python project adapters. Unsupported semantics remain explicit capability gaps. |
