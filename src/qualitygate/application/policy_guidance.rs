...
fn plan(failure: &PolicyInputError, context: &Context) -> Vec<NextStep> {
    ...
    match failure {
        ...
        PolicyInputError::SnapshotConfigurationMissing { .. } => {
            ...
            let message = if context.policy_ref {
                Some((
                    "select_policy_reference",
                    "Select a policy reference containing the reviewed configuration, or commit it and use the new reference.",
                ))
            } else {
                match selection {
...
fn initialize(root: &Path, config_path: &str) -> Option<NextStep> {
    ...
    let mut command = format!("qualitygate --root {}", shell_quote(root));
    if config_path != config::CONFIG_FILE {
        command.push_str(&format!(" --config {}", shell_quote(&config_path)));
    }
    command.push_str(" init --format json");
    Some(NextStep {
