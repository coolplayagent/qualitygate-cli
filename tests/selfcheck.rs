...
fn full_corpus_runs_real_evaluators_and_keeps_negative_cases_green_only_on_golden_agreement() {
    ...
    assert_eq!(cases.len(), 932, "Shipped fixture inventory changed");
    for suite in ["minimal", "typical", "stress"] {
        let case = cases
            .iter()
            .find(|case| case["suite"] == suite && case["fixture"] == "issue9-secret-fail")
            .unwrap();
        assert_eq!(
            case["input"],
        ...
        assert!(
            python["input"]
                .as_str()
                .unwrap()
                .contains("issue11-cases.json")
        );
        let shell = cases
            .iter()
            .find(|case| {
                case["suite"] == suite
                    && case["rule"] == "shell-missing-shebang"
                    && case["observed"]["verdict"] == "fail"
            })
            .unwrap();
        assert!(
            shell["input"]
                .as_str()
                .unwrap()
                .contains("issue12-cases.json")
        );
        let rust = cases
            .iter()
            .find(|case| {
                case["suite"] == suite
                    && case["rule"] == "rust-untrusted-dynamic-library-loading"
                    && case["observed"]["verdict"] == "fail"
            })
            .unwrap();
        assert!(
        ...
        let go = cases
            ...
            .find(|case| {
                case["suite"] == suite
                    && case["rule"] == "go-sql-injection"
                    && case["observed"]["verdict"] == "fail"
            })
            .unwrap();
        assert!(go["input"].as_str().unwrap().contains("issue15-cases.json"));
        let python_review = cases
            .iter()
            .find(|case| {
                case["suite"] == suite
                    && case["rule"] == "py-eval-exec"
                    && case["observed"]["verdict"] == "fail"
            })
            .unwrap();
        assert!(
            python_review["input"]
                .as_str()
                .unwrap()
                .contains("issue16-cases.json")
        );
        let c_family = cases
            .iter()
            .find(|case| {
                case["suite"] == suite
                    && case["rule"] == "c-array-safety"
                    && case["observed"]["verdict"] == "fail"
            })
            .unwrap();
        assert!(
            c_family["input"]
                .as_str()
                .unwrap()
                .contains("issue18-cases.json")
        );
        let cpp = cases
            .iter()
            .find(|case| {
                case["suite"] == suite
                    && case["rule"] == "cpp-no-realloc"
                    && case["observed"]["verdict"] == "fail"
            })
            .unwrap();
        assert!(
