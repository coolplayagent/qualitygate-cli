...
| Source vocabulary versus enforcement | A short archive label can make a scoped vendor example appear to be a universal requirement. | A source-local `controls` label only binds a rule claim to reviewed provenance. It is normalized archive vocabulary, not vendor text, and cannot change lifecycle status, severity, scope, or missing-evidence behavior. |
...
The registry's source-local `controls` are deliberately normalized for mapping
and are not claims that a source uses those exact terms. They prevent an
unreviewed control name from being attached to a source, while leaving the
repository's adopted policy and lifecycle evidence as the only paths to a
blocking result.
